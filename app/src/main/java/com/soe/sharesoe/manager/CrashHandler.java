...
/**
 * Created by zhangshili on 16/10/27.
 */
...
public class CrashHandler implements UncaughtExceptionHandler {
    ...
    private String getFilePath() {
        ...
        if (isSDCardExist && isRootDirExist) {
            file_dir = Environment.getExternalStorageDirectory().getAbsolutePath() + "/CooLQIcrashlog/";
        } else {
            // MyApplication.getInstance().getFilesDir()返回的路劲为/data/data/PACKAGE_NAME/files，其中的包就是我们建立的主Activity所在的包
            file_dir = mContext.getFilesDir().getAbsolutePath() + "/CooLQIcrashlog/";
        }
        return file_dir;
    }
    /**
