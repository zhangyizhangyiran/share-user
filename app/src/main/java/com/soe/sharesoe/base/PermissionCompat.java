 ...
 * </pre>
 */
public class PermissionCompat implements ActivityCompat.OnRequestPermissionsResultCallback {
    private String hint;
    ...
    public PermissionCompat(Activity mContext) {
    ...
    public boolean isPermissionGranted(String permissionName) {
        ...
        //判断是否需要请求允许权限
        int hasPermision = ActivityCompat.checkSelfPermission(mContext, permissionName);
    ...
    public List<String> isPermissionsAllGranted(String[] permArray) {
        ...
        for (int i = 0; permArray != null && i < permArray.length; i++) {
            if (PackageManager.PERMISSION_GRANTED != ActivityCompat.checkSelfPermission(mContext, permArray[i])) {
...
//                okPermissionResult(requestCode);
                ...
                ActivityCompat.requestPermissions(mContext, new String[]{s}, requestCode);
                flag = false;
            }
        }
...
//                okPermissionResult(requestCode);
                ...
                ActivityCompat.requestPermissions(mContext, perList.toArray(new String[perList.size()]), requestCode);
                flag = false;
            }
        }
        return flag;
